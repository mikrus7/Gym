            ...
            <?php  
                ...
                for($cb=1;$cb<=$boxDisplay;$cb++){ 
                    if(($cb >= $currentMonthFirstDay || $currentMonthFirstDay == 1) && $cb <= ($totalDaysOfMonthDisplay)){ 
                        ...
                        $result = $conn->query("SELECT eventsTitle FROM events WHERE eventsDate = '".$currentDate . "'"); 
...
function getEvents($date = ''){ 
    ...
    $result = $conn->query("SELECT * FROM events WHERE eventsDate = '".$date. "'"); 
    if($result->num_rows > 0){ 
        ...
        while($row = $result->fetch_assoc()){ $i++; 
            if($result->num_rows > 0){
                $resultDriver = $conn->query("SELECT eventsAttendanceDriverId FROM eventsattendance WHERE eventsAttendanceLinkedId='".$row['eventsId']."'");
                if($resultDriver->num_rows>0){
                $rowDriver= $resultDriver->fetch_assoc();
                if($rowDriver['eventsAttendanceDriverId'] == $_SESSION['idUser']){
                    $eventListHTML .= '<li class="sidebar__list-item"><span class="list-item__time">'.$i.'.</span>'.$row['eventsTitle']. " You're attending</li>"; 
                }else{
                    $eventListHTML .= '<li class="sidebar__list-item"><span class="list-item__time">'.$i.'.</span>'.$row['eventsTitle']. ' ' ?><form method="post" action="includes/attend.inc.php"><input type="hidden" name="event" id="event" value=<?php echo $row['eventsId'];?>><input type="hidden" name="driver" id="driver" value=<?php echo $_SESSION["idUser"];?>><button type="submit" name="submit" value="Attend">Attend</button></form><?php '</li>';
                }
            }else{
                $eventListHTML .= '<li class="sidebar__list-item"><span class="list-item__time">'.$i.'.</span>'.$row['eventsTitle']. ' ' ?><form method="post" action="includes/attend.inc.php"><input type="hidden" name="event" id="event" value=<?php echo $row['eventsId'];?>><input type="hidden" name="driver" id="driver" value=<?php echo $_SESSION["idUser"];?>><button type="submit" name="submit" value="Attend">Attend</button></form><?php '</li>';
                } 
